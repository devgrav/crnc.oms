...
- **`.WebApi`** — ASP.NET Core host: `Startup.cs` wires DI, EF `DbContext`, MassTransit bus + consumers, JWT auth, NSwag/Swagger, Prometheus metrics, health checks (`/health`). `Controllers/` call the `ICommandQueryDispatcher`; `Consumers/` handle inbound MassTransit messages; `Authorization/` has role-based policy handlers. **Security, Sales and Production are the exception** (post-.NET 10 migration): minimal hosting in `Program.cs`, no `Startup.cs`; `Authorization/` only has role constants/`AuthSettings` and `CurrentUserContext`, no policy handlers (Production's `JobsController` is a bare `[Authorize]` with no role checks at all). Security additionally has no MassTransit (pure inbound HTTP); Sales and Production both keep MassTransit (RabbitMQ, MassTransit 8.x) — Sales because it both publishes and consumes workflow events, Production because it has no HTTP endpoint to create a job at all: the only way in is consuming `OrderConvertedToJobEvent`.
...
Test coverage today: `Crnc.Oms.Sales.Tests` (Sales `Domain` unit tests), `Crnc.Oms.Security.E2ETests`, `Crnc.Oms.Sales.E2ETests` and `Crnc.Oms.Production.E2ETests` (those services over HTTP/messaging, via Testcontainers). Production has no `Domain` unit test project yet (the convention below expects one eventually); Notification.* has neither kind. See "Test conventions" below.
...
`Crnc.Oms.Security`, `Crnc.Oms.Sales` and `Crnc.Oms.Production` were migrated to .NET 10 (`net10.0` across all app projects; Security and Sales also carry the migration into their own `.Tests` project, Production has no `.Tests` project yet) — Notification.* is still on `netcoreapp3.1`; building any migrated `.sln` with the .NET 10 SDK is expected to show `NETSDK1138` warnings for the still-3.1 Notification solutions, not for the migrated services' own projects. All three migrated services' e2e test projects run on `net10.0` regardless of the API's own TFM (they drive the service over HTTP/messaging through a container, not via `ProjectReference` to the service itself — see "Test conventions" above for Production's one narrow exception), and all three are members of their service's `.sln` (safe because each Dockerfile restores/publishes an explicit `.csproj`, not the whole solution):
```
