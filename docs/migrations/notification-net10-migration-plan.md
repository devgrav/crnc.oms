...
### Риск 5 закрыт: сборка SPA починена, CORS хаба проверен

Изначально этот раздел назывался «что не проверялось»: SPA не собиралась из-за `yarn: not found`, и порядок `UseCors` до `MapHub` оставался единственным непроверенным риском. Причина оказалась не в машине и не во фронтенде, а в плавающем теге: `src/Client/Dockerfile` собирался на `node:alpine`, который сегодня резолвится в Node 26 — там yarn больше не поставляется вовсе, а OpenSSL 3 убрал хеш `md4`, на который опирается webpack 3. Та же болезнь, что у `dotnet/core/aspnet:3.1`, только с другого конца. Пин на `node:16-alpine` (последний LTS эпохи этого фронтенда, yarn 1.22, формат `yarn.lock` v1) чинит сборку одной строкой.

После этого риск 5 проверен напрямую — браузер для этого не нужен, он полагается ровно на эти заголовки:

| Запрос к `/hubs/push/negotiate` | Результат |
|---|---|
| preflight, `Origin: http://localhost:8092` | 204, `Allow-Origin: http://localhost:8092`, `Allow-Credentials: true`, `Allow-Methods: POST` |
| реальный negotiate с тем же origin | 200, `Allow-Origin` и `Allow-Credentials` на месте |
| preflight, `Origin: http://evil.example` | 204 **без** CORS-заголовков — браузер такой запрос заблокирует |

Хаб отдаёт CORS-заголовки, значит `UseCors` действительно отрабатывает до `MapHub`. Непроверенных рисков в плане не осталось.
