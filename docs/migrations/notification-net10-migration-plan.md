...
Отдельно: у контекста **два предсуществующих дефекта, из-за которых сквозной сценарий уведомлений сейчас не работает** (§«Блокер №0» и §9). Это меняет смысл фазы e2e-тестов: она фиксирует не «правильное поведение до миграции», а **фактическое, включая сломанное**, — ровно так, как в Production фиксировались 500 вместо 404.
...
11. **Security в тестах заменяется на WireMock** — по образцу набора Sales, включая тот же сетевой алиас и порт. **Стаб обязан требовать заголовок `Authorization`** — иначе набор был бы зелёным на сломанной цепочке (§«Блокер №0»).
...
14. **Предсуществующие баги (§9) чиним отдельными коммитами в фазе 5**, а не внутри коммита миграции. Поведение, которое e2e фиксируют как текущее (сломанная цепочка уведомлений), меняем **после** миграции, чтобы baseline оставался baseline'ом.

---

## Блокер №0: Gateway ходит в Security без токена — цепочка уведомлений сломана с 2020-03-04

`Crnc.Oms.Notification.Gateway.Integration/Gateways/UserInfoGateway.cs` дёргает `GET /api/users/{id}` у Security через `new RestClient(...)` **без аутентификатора и без заголовка `Authorization`**. `Crnc.Oms.Security.WebApi/Controllers/UsersController` объявлен как `[Authorize]` на уровне класса. Значит вызов получает 401, `UserInfoGateway` бросает `Exception`, `NotificationService.SendNotificationToUserAsync` падает **до** обращения к email- и push-шлюзам — ни письма, ни пуша не будет ни по HTTP-входу (`POST /api/notifications/user` → 500), ни по входу из шины (консьюмер уходит в retry и затем в `sendNotificationToUser_error`).

История подтверждает, что это регрессия, а не задумка:

- `d655cac` (2020-02-19, «Добавлен mass transit и нотификация через брокер») — в тексте коммита прямо написано: *«Временно удалена авторизация в security api, так как обмен сообщениям не предполагает авторизации»*. В этой ревизии `[Authorize]` на классе `UsersController` действительно нет.
- `4bf86e1` (2020-03-04, «Изменения в контрактах обмена сообщениями») — `[Authorize]` вернулся на класс. `UserInfoGateway` при этом не изменился и токен посылать не начал.

С тех пор код обеих сторон не трогали. Утверждение README о том, что `shon_bean` получает пуши при смене статуса заказа, относится к состоянию до марта 2020 года.

**Что это значит для миграции.** Ничего не блокирует технически — но определяет содержание фазы 1: e2e-набор пишется под фактическое поведение (Gateway возвращает 500 и не шлёт команд в шину), а не под желаемое. Иначе тесты придётся переписывать дважды.

**Как чинить (фаза 5, отдельным коммитом).** Три варианта, в порядке предпочтения:

1. **Сервисный аккаунт с кэшем токена** (рекомендуется). `UserInfoGateway` получает токен у Security сам — ровно тем же способом, каким это уже делает `Push.Client.Auth.AuthClient` (`POST /api/accounts/auth` с логином/паролем из конфигурации), и кэширует его до истечения. Плюсы: не меняет ни контракты сообщений, ни Sales, работает одинаково на HTTP- и на шинном входе. Минус: учётные данные в конфигурации — но такой прецедент в репозитории уже есть (`notification-push-client`).
2. **Прокидывать токен вызывающего** через `IHttpContextAccessor` (он уже зарегистрирован в `Startup` Gateway и не используется). Закрывает только HTTP-вход; у консьюмера шины входящего токена нет, так что вариант неполный и всё равно требует №1 как запасной путь.
3. **Класть email получателя прямо в `SendNotificationToUserCommand`.** Архитектурно самое чистое — Gateway перестаёт ходить в Security вообще, а Sales этот email уже знает (`Manager.Email`). Но это изменение контракта сразу в двух контекстах, то есть за пределами миграции Notification.

Решение по варианту принимается перед фазой 5, а не сейчас: до неё нужно увидеть фактическое поведение на живом стенде (шаг 1 фазы 0).
...
1. **`docker-compose --profile notification up` + смена статуса заказа в Sales.** Смотрим: в RabbitMQ UI (`localhost:15673`) появилась ли очередь `sendNotificationToUser_error`; в логах `crnc-oms-notification-gateway-api` — 401 от Security; дошло ли что-нибудь до `notification-push-api`. Это подтверждает (или опровергает) §«Блокер №0» фактом, а не рассуждением.
...
Важная деталь: **стаб Security должен требовать `Authorization`**. WireMock-маппинг на `GET /api/users/{id}` ставится с `headers: { Authorization: { matches: "Bearer .*" } }`, а «дефолтный» ответ на тот же путь без заголовка — 401. Без этого тест на цепочку уведомлений был бы зелёным при живом «Блокере №0», то есть проверял бы стенд, а не сервис.

### Что покрываем

**Gateway** (`GatewayNotificationsTests`)
- `POST /api/notifications/user` без токена → 401.
- С валидным токеном и известным пользователем → **фактическое поведение baseline** (ожидается 500 из-за «Блокера №0»; тест помечен ссылкой на этот раздел и переписывается в фазе 5 вместе с фиксом).
- Невалидное тело (пустой `message`) → 400, и **ключи `ModelState` в ответе в camelCase** — это тест на §4.
- `GET /__admin/requests` подтверждает, что вызов в Security состоялся (то есть 500 приходит именно оттуда, а не из более ранней точки).
...
**Токен в `UserInfoGateway` на этом шаге не появляется.** Переписывание на `HttpClient` — эквивалентная замена транспорта; добавление заголовка `Authorization` меняет поведение и относится к фазе 5 («Блокер №0»).
...
- **«Блокер №0»** — `UserInfoGateway` без токена. Вместе с фиксом переписывается e2e-тест, зафиксировавший 500.
...
1. Три проверки на живом стенде из раздела «Фаза 0»: сломанная цепочка уведомлений, поведение HTTP-входа Email, отсутствие `/health`. Результаты дописать в этот план.
...
4. Зафиксировать в тестах фактическое (сломанное) поведение цепочки уведомлений со ссылкой на §«Блокер №0».
...
14. **Ручной сквозной сценарий через `docker-compose up`** (профиль `client`): логин в SPA, смена статуса заказа в Sales → команда доходит до Gateway. С учётом «Блокера №0» ожидаемый результат — сообщение в `sendNotificationToUser_error`, а не пуш; важно, что это **то же самое**, что было на 3.1, и что цепочка не деградировала сильнее.
...
19. Фикс «Блокера №0» по выбранному варианту + переписывание e2e-теста шага 4.
20. Фикс `SendEmailNotificationToReceiverConsumer` (теряются `SenderEmail`/`ReceiverEmail`), с тестом.
21. По желанию — подключение `MonitoringRequestMiddleware` во всех трёх и/или чистка мёртвых `IntegrationEndpoints` из compose. Оба — самостоятельные решения, не хвост миграции.
...
- Там же — про **SignalR-клиент внутри тестового процесса** как способ наблюдать доставку пуша, и про **требование `Authorization` в стабе WireMock** (иначе набор зелёный на сломанной цепочке).
...
- Снять/поправить утверждение README и AGENTS.md о том, что `shon_bean` получает push-уведомления при смене статуса заказа, — до фикса «Блокера №0» это неправда.
...
| 12 | **E2E зелёные на сломанной цепочке**, потому что WireMock отвечает 200 без токена | Предотвращается конструкцией стаба (решение 11): маппинг требует `Authorization`, дефолт — 401 |
...
- `Crnc.Oms.Notification.Gateway.Integration/Gateways/UserInfoGateway.cs` — RestSharp → `HttpClient` (§6); токен — **фаза 5**
...
- `AGENTS.md` (§12), `README.md` — утверждение про пуши `shon_bean`
